...
void UART5_IRQHandler(void)
{
    ...
    else if(UART5->SR & UART_FLAG_IDLE)
    {
        ...
        if ((hdma_uart5_rx.Instance->CR & DMA_SxCR_CT) == RESET)
        {
            ...
            this_time_rx_len = DBUS_RX_BUF_NUM - hdma_uart5_rx.Instance->NDTR;

            //重新设定数据长度
            hdma_uart5_rx.Instance->NDTR = DBUS_RX_BUF_NUM;
...
rc_dbus_obj_t *dbus_rc_init(void)
{
    ...
    rc_doub_dma_init(dbus_rx_buf[0], dbus_rx_buf[1], DBUS_RX_BUF_NUM);
