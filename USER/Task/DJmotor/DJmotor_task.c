...
/* -------------------------------- 线程间通讯Topics相关 ------------------------------- */
/* -------------------------------- 调试监测线程相关 --------------------------------- */
static uint32_t DJmotor_task_dwt = 0;   // 毫秒监测
static float DJmotor_task_dt = 0;       // 线程实际运行时间dt
static float DJmotor_task_delta = 0;    // 监测线程运行时间
static float DJmotor_task_start_dt = 0; // 监测线程开始时间
/* -------------------------------- 调试监测线程相关 --------------------------------- */
...
/* -------------------------------- 线程间Topics初始化 ------------------------------- */
/* -------------------------------- 调试监测线程调度 --------------------------------- */
    DJmotor_task_dt = dwt_get_delta(&DJmotor_task_dwt);
    DJmotor_task_start_dt = dwt_get_time_ms();
/* -------------------------------- 调试监测线程调度 --------------------------------- */
    for(;;)
    {
/* -------------------------------- 调试监测线程调度 --------------------------------- */
        DJmotor_task_delta = dwt_get_time_ms() - DJmotor_task_start_dt;
        DJmotor_task_start_dt = dwt_get_time_ms();
        DJmotor_task_dt = dwt_get_delta(&DJmotor_task_dwt);
/* -------------------------------- 调试监测线程调度 --------------------------------- */
/* -------------------------------- 线程订阅Topics信息 ------------------------------- */
